...
Процедура Инициализировать()
	...
	// Попытка
	Лог = Логирование.ПолучитьЛог(ИмяПроекта());
	...
	ВыводПротоколаВФайл.ОткрытьФайл(ОбъединитьПути("log", "storage-release.log"));
	// Лог.мВложенныеЛоги.Добавить(ВыводПротоколаВФайл);
...
Процедура ВывестиСтрокуЗаголовка()
	...
	КонецПопытки;
	Сообщить(Наименование + ВтораяЧасть);	Сообщить(ПолучитьСтрокуСимволов("`", ШиринаКонсоли-1));
КонецПроцедуры
...
Функция ПрочитатьФайл(Знач ИмяФайла, Знач Кодировка = Неопределено) Экспорт
	Лог.Отладка("Читаю текст из файла " + ИмяФайла);
...
Функция ПрочитатьФайлJSON(Знач ИмяФайла) Экспорт
	Лог.Отладка("Получаю объект данных из json файла " + ИмяФайла);
...
Процедура ПолучитьОтчет()
	...
	ИмяИБ = СтрЗаменить(Настройки["ib"], "mc_", "");
	...
	ИмяФайлаОтчета = ОбъединитьПути(КаталогРабочихФайлов, ПостфиксИмениФайла + ?(РежимРасширение, "_ext", "") + "" + СтрокаДиапазонаВерсий + ".REPORT" + ".mxl");
