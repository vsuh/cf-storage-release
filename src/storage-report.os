...
Процедура ПолучитьОтчет()
	...
	Иначе
		...
		Попытка
			НомерНачВерсииИзКонфигурации = Настройки["version"];
		Исключение
			НомерНачВерсииИзКонфигурации = 1;
		КонецПопытки;
		Попытка
			НомКонВерсииИзКонфигурации = Настройки["version-last"];
		Исключение
			НомКонВерсииИзКонфигурации = 1;
		КонецПопытки;
		
		Если НомерНачВерсииИзКонфигурации > 1 Тогда
			ОбрезатьТаблицуПоНачальномуНомеруВерсии(ТаблицаВерсий, НомерНачВерсииИзКонфигурации);
		КонецЕсли;
		...
		Если ТаблицаВерсий.Количество() = 0 Тогда
			НомПоследнейВерсииВХранилище = НомКонВерсииИзКонфигурации;
		Иначе
		...
		СтрокаДиапазонаВерсий = "_[" + НомерНачВерсииИзКонфигурации + "-" + НомПоследнейВерсииВХранилище + "]";
		...
		Лог.Информация("Формирую отчет по версиям. с " + НомерНачВерсииИзКонфигурации + " по " + НомПоследнейВерсииВХранилище + " в " + ИмяФайлаОтчета);
		ХранилищеКонфигурации.ПолучитьОтчетПоВерсиям(ИмяФайлаОтчета
			, НомерНачВерсииИзКонфигурации, НомПоследнейВерсииВХранилище);
		Если НомерНачВерсииИзКонфигурации = НомПоследнейВерсииВХранилище Тогда
	...
	Если ИсправительныйРелиз Тогда // В каждый отчет по испр. релизу включать все изменения с начала исправлений
		НомерНачВер = НомерНачВерсииИзКонфигурации;
		НомерКонВер = НомПоследнейВерсииВХранилище;
	Иначе
		НомерНачВер = НомКонВерсииИзКонфигурации;
		НомерКонВер = НомПоследнейВерсииВХранилище;
	КонецЕсли;
