...
Функция ПолучитьСтрокуСледующегоРелиза(СтрокаТекущегоРелиза)
	...
	Иначе
		МажорнаяЧастьНомера = 1 + Число(МассивЧастей[0]);
		МинорнаяЧастьНомера = 1 + Число(МассивЧастей[1]);
...
Процедура ПолучитьОтчет()
	...
	КаталогРабочихФайлов = ОбъединитьПути(РабочийКаталог, ИмяИБ, ПредставлениеНомераРелиза(ущегоРелиза));
	...
	Иначе
		...
		СтрокаНомераРелиза = ПредставлениеНомераРелиза(ущегоРелиза);
	...
	КонецЕсли;
	ЗаписатьИзмененияВНастройки(НомерПоследнейВерсии, СтрокаТекущегоРелиза);
